...
{{
    ...
    start: start base position or interval;  an array of one or two numbers, if unsure use "unknown",
    end: end base position of interval; an array of one or two numbers, if unsure use "unknown",
}}
...
def extract_object(query, template, model, llm_json):
...
def get_extractor_chain(template, model, llm_json):
    parser = PydanticOutputParser(pydantic_object=model)
    template = ChatPromptTemplate.from_template(
        template,
        partial_variables={"format_instructions": parser.get_format_instructions()},
...
def _get_validator_chain(llm_json):
    template = ChatPromptTemplate.from_template(
...
def get_validate_and_followup_chain(llm_text):
...
"""
    ...
    return _get_validator_chain(llm_text) | RunnableLambda(invalid_followup)


def get_data_extractor_map_chain(llm_json):
    extractor_chain = {
        "scope": get_extractor_chain(scope_extractor_template, Scope, llm_json),
        "filters": get_extractor_chain(filter_extractor_template, Filters, llm_json),
        "variant": get_extractor_chain(variant_extractor_template, Variant, llm_json),
        "granularity": get_extractor_chain(
            granularity_extractor_template, Granularity, llm_json
        ),
        "query": RunnablePassthrough(),
    }
...
def get_success_followup_chain(llm_text):
...
def get_analytics_code_generator_chain(llm_json):
    return get_extractor_chain(
        analytics_code_generator_template, GeneratedCodeAnalytics, llm_json
    )
